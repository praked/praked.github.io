...
**Figure S3.** *Median paired ΔA<sub>hull</sub> as a function of γ for all four wind conditions. Negative is better. Note the asymmetric failure modes: at γ = −2 the flock stretches (largest in 10 m/s + gusts); at γ = 10 cohesion reverses sign and the flock disperses under steady 10 m/s wind.*
...
These figures extend the main-text paired-difference analysis (Fig. 2, reported for the 10 m/s environments) to all three metrics across all four wind conditions. Each point is the median paired difference (treatment − baseline) over 50 seed-matched runs; error bars show the IQR. Red points are significant improvements under Holm-corrected Wilcoxon (p < 0.05).
 
![Median paired Δ flock area vs γ, all four wind conditions](/assets/img/SailSwarmSim/median_delta_area.png "Median paired Δ flock area vs γ"){:.lead}
**Figure S4.** *Median paired Δ flock area vs. γ. Negative is better (tighter flock). Significant tightening concentrates at small-to-moderate γ; strong fast-neighbor following (γ ≤ −1.5) and over-anchoring (γ = 10) inflate the flock, most severely under 10 m/s wind.*
 
![Median paired Δ cumulative collisions vs γ, all four wind conditions](/assets/img/SailSwarmSim/median_delta_collisions.png "Median paired Δ collisions vs γ"){:.lead}
**Figure S5.** *Median paired Δ cumulative collisions vs. γ. Negative is better (fewer collisions). Significant reductions concentrate at large γ (notably γ = 10) and in the higher-energy 10 m/s and gusty environments; at small γ the change is mostly not significant.*
 
![Median paired Δ polarisation vs γ, all four wind conditions](/assets/img/SailSwarmSim/median_delta_polarisation.png "Median paired Δ polarisation vs γ"){:.lead}
**Figure S6.** *Median paired Δ polarisation vs. γ. Positive is better (stronger alignment). Alignment follows a clear inverted-U: a central band of small γ gives significant gains in all four environments, while strong fast-following (γ ≤ −1.5) and strong anchoring (γ ≳ 1) both reduce alignment.*
...
<!-- Ablation figure: three panels (cohesion / alignment / safety); solid = luffing, hatched = no luffing -->
 
![Ablation: sail luffing vs. speed-weighting across cohesion, alignment, and safety](/assets/img/SailSwarmSim/ablation_luffing.png "Ablation isolating the luffing layer from speed-weighting"){:.lead}
**Figure S7.** *Ablation isolating the sail-luffing layer. Cohesion (top, flock area — lower is tighter), alignment (middle — higher is better), and safety (bottom — lower is safer) across the four wind conditions. For each γ, the solid box is with luffing and the adjacent hatched box is the same γ with luffing removed. Removing luffing widens the flock and raises unsafe-proximity counts — most visibly in the 10 m/s and gusty environments — while the γ-ordering (the inverted-U in alignment, the rise in area toward γ = 10) persists with luffing held constant, isolating that structure as the speed-weighting contribution.*
